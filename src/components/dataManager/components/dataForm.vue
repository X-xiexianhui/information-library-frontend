  ...
  >
    ...
    >
      ...
      <vxe-column field="value" title="数据" :edit-render="{}">
        <template #default="{ row }">
          <vxe-switch v-if="row.data_type === 'boolean'" open-label="是"></vxe-switch>
          <div v-else>
            <span>{{ row.value }}</span>
        ...
        <template #edit="{row}">
          ...
          <vxe-input v-else-if="row.data_type === 'time'||row.data_type === 'date'||row.data_type === 'datetime'" :type="row.data_type"></vxe-input>
